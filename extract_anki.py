...
def parse_html(html_content):
    ...
    if lines:
        ...
        if '[' not in first_line:
            print(f"DEBUG: Entry without expected [part_of_speech] format: '{first_line}'")
    ...
    for line in lines[1:]:
        ...
        if any(x in line for x in ['Audio', 'Dictionary Entry', 'Example Sentences', 'Video Example Search', 'Flash cards by']):
            continue
    ...
    for (flds,) in cursor.fetchall():
        ...
        if parsed['content']['word']:  # Only include entries that have a word
            entry = {
                ...
                'audio': parsed['audio'],
                'dictionary_url': parsed['links']['dictionary'][0] if parsed['links']['dictionary'] else '',
                'examples_url': parsed['links']['examples'][0] if parsed['links']['examples'] else '',
                'video_url': parsed['links']['video'][0] if parsed['links']['video'] else ''
            }
